...
import { getEffectiveProduct, getPackageOutputFolderName, type ProductConfiguration } from './lib/productProfile.ts';
...
function packageTask(platform: string, arch: string, sourceFolderName: string, destinationFolderName: string, _opts?: { stats?: boolean }) {
	...
	const task = () => {
		...
		const quality = (product as ProductConfiguration & { quality?: string }).quality;
		...
		if (platform === 'win32') {
			...
			if (quality === 'stable' || quality === 'insider') {
				...
				result = es.merge(result, gulp.src('resources/win32/appx/AppxManifest.xml', { base: '.' })
					...
					.pipe(replace('@@FileExplorerContextMenuCLSID@@', (product as ProductConfiguration & { win32ContextMenu?: Record<string, { clsid: string }> }).win32ContextMenu![arch].clsid))
