...
const extendedProduct = product as ProductConfiguration & { darwinCredits?: string; electronRepository?: string };
...
export const config = {
	...
	tag: extendedProduct.electronRepository ? `v${electronVersion}-${msBuildId}` : undefined,
	...
	...(darwinCreditsTemplate ? { darwinCredits: Buffer.from(darwinCreditsTemplate({ commit: String(commit ?? ''), date: new Date().toISOString() })) } : {}),
	linuxExecutableName: product.applicationName,
	winIcon: 'resources/win32/code.ico',
	...(process.env['GITHUB_TOKEN'] ? { token: process.env['GITHUB_TOKEN'] } : {}),
	...(extendedProduct.electronRepository ? { repo: extendedProduct.electronRepository } : {}),
